define(function(require,exports,module) {
    exports.wode = function() {
        ...
        function musetewande() {
            ...
            for (let i = 0; i < forms.length; i++) {
                forms.eq(i).form({
                    ...
                    success: function(data) {
                        bodeschuanmu++;
                        ersacadddczcasdasdwd.push(data);
                        ...
                        addyinxunObj = {
                            ...
                            money:$('#zx9').val(),
                            synopsis:$('#zx10').val(),
                            indexFile: wodeboschuanmu,
                            file: JSON.stringify(ersacadddczcasdasdwd)
                        }

                        console.log(addyinxunObj)

                        // if (i == (forms.length - 1)) {
                            if (bodeschuanmu == forms.length) {
                                $.ajax({
                                    type: 'POST',
                                    async:false,
                                    url: '/yinxun/add',
        ...
        function addyinxunImgs(){  
            ...
            // var musexinlyq=$("<div>").css({"overflow":"hidden","lineHeight":"45px"});
            // var nwesimgs_btn=$("<input>").attr({type:"file",multiple:"multiple",name:"IMG"}).css("border","1px solid black");
            var _id = "formTest" + index++;

            var musexinlyq = '<form class="formClass" id="' + _id + '" action="/upload" method="POST" enctype="multipart/form-data">'+'<input class="fileClass" type="file" name="file" multiple="multiple">'+
            '</form>';
            $("#baddForm").append(musexinlyq);
            ...
            // musexinlyq.append(nwesimgs_btn);
